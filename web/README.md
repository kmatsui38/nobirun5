...
| パス | 画面 | 状態 |
|---|---|---|
| `/` | ホーム（S2）: ストリーク・今日の復習への導線（中断中は「つづきから」）・ログアウト | 実装済み |
| `/login` | ログイン（S1） | 実装済み |
| `/session` | 復習セッション（S3/S4）: 出題→採点→解説→完了。メモ欄・中断/再開・所要時間の計測 | 実装済み |
...
|---|---|
| 2-1 | ひと目でわかる進捗サマリ |
| 2-2b | 学習時間（開始・終了・実学習時間・中断回数） |
| 2-2c | 1問あたりの所要時間（単元ごと。時間がかかる＝苦手のサイン） |
| 3-1 | 解答履歴の全件（所要時間・メモ込み。CSV出力用） |
| 4-3 | 1日の出題数を変える |
| 4-4 | 生徒が書いたメモを読む |
